...
// ─── Storage keys ─────────────────────────────────────────────────────────────
const SESSIONS_KEY = 'ai_firm_sessions';
...
const SOURCE_LABELS = {
  ...
  tasks: 'Tasks', invoices: 'Invoices', events: 'Hearings', notes: 'Notes',
  client: 'Client Profile', lawyer: 'Team Member',
};
const SOURCE_COLORS = {
  case_meta: '#3B82F6', document: '#16A34A', timeline: '#9333EA',
  tasks: '#D97706', invoices: '#DC2626', events: '#0891B2', notes: '#7C3AED',
  client: '#EC4899', lawyer: '#0F766E',
};
...
const IndexBadge = ({ status, indexing, onReindex }) => {
  ...
  return (
    <View style={ib.row}>
      {indexing ? (
        <><ActivityIndicator size="small" color={C.amber600} /><Text style={[ib.txt, { color: C.amber600 }]}>Updating knowledge base…</Text></>
      ) : status?.is_indexed ? (
        ...
        <><View style={[ib.dot, { backgroundColor: C.amber600 }]} /><Text style={[ib.txt, { color: C.amber600 }]}>Preparing knowledge base…</Text></>
      )}
      {!indexing && (
...
const EmptyState = ({ onSuggestion }) => (
  <View style={es.wrap}>
    <View style={es.iconWrap}><FontAwesome5 name="building" size={30} color={C.primary} /></View>
    <Text style={es.title}>Firm AI Assistant</Text>
    <Text style={es.sub}>Ask anything about your clients, cases, invoices, tasks, and documents.</Text>
...
const SessionCard = ({ session, onPress, onDelete, onRename, isActive }) => {
  ...
  return (
    <TouchableOpacity style={[sc.card, isActive && sc.cardActive]} onPress={onPress} activeOpacity={0.8}>
...
export default function AIAssistantScreen({ navigation }) {
  ...
  const saveSessions = useCallback(async (list) => {
    try { await AsyncStorage.setItem(SESSIONS_KEY, JSON.stringify(list)); } catch {}
  }, []);
  ...
  const updateSessionMeta = useCallback((sessionId, updates) => {
    setSessions(prev => {
      const next = prev.map(s => s.id === sessionId ? { ...s, ...updates, updatedAt: new Date().toISOString() } : s);
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      const res = await ragAPI.firmAsk(q, history);
      const aiMsg = {
  ...
  }, [question, loading, messages, activeSessionId, createSession, saveMsgs, updateSessionMeta]);
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.header}>
        ...
        <View style={s.aiIconWrap}>
          <FontAwesome5 name="building" size={16} color={C.white} />
        </View>
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={s.headerTitle}>Firm AI Assistant</Text>
          <Text style={s.headerSub}>All cases · All clients · All data</Text>
        </View>
        <TouchableOpacity style={s.newHeaderBtn} onPress={newSession} activeOpacity={0.8}>
          <FontAwesome5 name="plus" size={11} color={C.white} />
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        {tab === 'history' ? (
          ...
          <View style={{ flex: 1 }}>
            ...
            <IndexBadge status={status} indexing={indexing} onReindex={triggerIndex} />
            ...
            <View style={s.inputBar}>
              <TextInput
                ...
                placeholder="Ask about your firm…"
...
const s = StyleSheet.create({
  ...
  aiIconWrap:   { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center' },
  headerTitle:  { fontSize: 16, fontWeight: '800', color: C.white },
  headerSub:    { fontSize: 11, color: 'rgba(255,255,255,0.7)', marginTop: 1 },
  newHeaderBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },

...
export class NewCaseModal {
  ...
  get step1Valid() { return this.f1().title.trim().length > 0 && this.f1().caseType.length > 0 && this.f1().priority.length > 0; }
  get step2Valid() { return this.f2().clientId.trim().length > 0; }
  get step3Valid() { return this.f3().billingType.length > 0 && String(this.f3().caseValue).trim().length > 0 && Number(this.f3().caseValue) >= 0; }
  ...
  openModal(clientId = '', clientName = '') {
    ...
    this.lockedClient.set(clientId ? { id: clientId, name: clientName } : null);

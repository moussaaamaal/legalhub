...
interface NewEventForm {
  ...
  location: string; caseRef: string; participants: { id: string; name: string }[];
...
export class NewEventModal implements OnInit {
  ...
  async ngOnInit() {
    await Promise.all([this._loadCases(), this._loadStaff()]);
  }
  ...
  private async _loadStaff() {
    if (this.staffSvc.staff().length > 0) return;
    await this.staffSvc.loadStaff();
    this.availableParticipants.set(
      this.staffSvc.staff().map(s => ({ id: s.id, name: s.name }))
    );
  }
  ...
  openModal(date?: string, hour?: number) {
    ...
    // Reset participants to all firm staff (no case selected yet)
    this.availableParticipants.set(
      this.staffSvc.staff().map(s => ({ id: s.id, name: s.name }))
    );
  ...
  async onCaseRefChange(caseId: string) {
    ...
    try {
      if (caseId) {
        const raw = await this.caseSvc.fetchTeam(caseId);
        this.availableParticipants.set(
          raw
            .map(r => ({
              id:   String((r['app_user'] as Record<string, unknown>)?.['id']        ?? r['user_id'] ?? ''),
              name: String((r['app_user'] as Record<string, unknown>)?.['full_name'] ?? ''),
            }))
